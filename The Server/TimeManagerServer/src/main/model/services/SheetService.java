...
public class SheetService 
{
	...
	public JSONObject getWeeklyChange(int userId,String date,int labelid)
	{
        ...
        for(int i = 0; i < 7; i++)
        {
        	...
        	{
        		if(timeSharings.get(j).getDate().equals(timeOfDate.get(Calendar.YEAR) + "年" + timeOfDate.get(Calendar.MONTH)+1 + "月" +timeOfDate.get(Calendar.DATE) + "日"));
        		{
  	...
  	public JSONObject getWeeklySheet(int userId,String date)
  	{
        ...
        for(int i = 0; i < labels.size(); i++)
        {
        	...
        	for(int j = 0; j < 7; j++)
            {
            	...
            	{
            		if(timeSharings.get(k).getDate().equals(timeOfDate.get(Calendar.YEAR) + "年" + timeOfDate.get(Calendar.MONTH)+1 + "月" +timeOfDate.get(Calendar.DATE) + "日"));
            		{
